...
for i in arr:
    # size = len(i)
    # if(size%2==0) : # i가 짝수 글자이면 반으로 나눠서 앞의 글자를 revert해서 뒤에거랑 일치하면 YES 아니면 NO
    #     front = i[0:size//2]
    #     back = i[(size//2):]
    #     validateEqual(front,back)

    # else: # i가 홀수글자이면 중간 단어를 기준으로 앞에거 revert한거랑 뒤에거랑 일치하면 YES, 아니면 NO
    #     front = i[0:(size//2)]
    #     back = i[(size//2)+1:]
    #     validateEqual(front,back)
